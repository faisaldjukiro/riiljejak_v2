...
<body>
                ...
                <div class="row">
                    ...
                    <div class="isal-container">
                        <div class="left-column">
                            ...
                            <ul class="bottom-column back-pagination mt-30" id="pagination"></ul>
                        </div>
    ...
    $(document).ready(function() {
        ...
        if (keyword) {
            $('#keyword').val(keyword);
            loadBerita(keyword, 1);
        }

        function loadBerita(keyword, page) {
            $.ajax({
                ...
                success: function(response) {
                    ...
                    if (response.berita.length > 0) {
                        $.each(response.berita, function(index, item) {
                            beritaHtml += `
                            <div class="col-lg-12 isal-item">
                                <div class="image-wrapper">
                                    <a href="<?= base_url() ?>${item.url}">
                                        <img src="<?= base_url('img/berita/') ?>${item.gambar}" 
                                             alt="image" 
                                             style="width: 1024px !important; height: 200px !important; object-fit: cover !important;">
                                    </a>
                                </div>
                                <div class="back-btm-content">
                                    <span class="viral-label">
                                        <a href="<?= base_url() ?>${item.url_kategori}" class="back-cates">${item.nm_kategori_master}</a>
                                    </span>
                                    <span class="date">${item.tgl_beritaa}</span>
                                    <h3>
                                        <a href="<?= base_url() ?>${item.url}">${item.judul}</a>
                                    </h3>
                                </div>
                            </div>
                        `;
